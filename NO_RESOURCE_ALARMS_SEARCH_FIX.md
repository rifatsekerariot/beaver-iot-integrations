...
## Yapılacaklar (beaver-iot / platformariot / monolith repoda)
...
Bu döküman `NoResourceFoundException` (404-benzeri, resource handler) için. Controller çalışıp `AlarmRepository`/`t_alarm` yoksa **500** alırsınız; bu Liquibase/`alarm.sql` ve `DB_TYPE=postgres` ile ilgili ayrı konudur (ör. `DASHBOARD_ID_UNDEFINED_FIX` veya `alarm.sql` + `verify-alarm-postgres.ps1`).
...
```powershell
# Örn. beaver-iot-docker / compose ortamında
...
Bu değişiklikler **beaver-iot** (veya **platformariot**) ve ilgili **Docker/nginx** repolarında yapılmalıdır; **beaver-iot-integrations** (bu repo) sadece entegrasyon modüllerini içerir, AlarmsController burada yoktur.
